        ...
        <div class="card-header">
            <div class="row">
                <div class="col-md-12 d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex">
                        @if($btnWorkOperation)
                        <button class="button btn btn-warning m-2" id="#" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Add Work Performed</button>
                        @else
                        @endif
                        @if($btnInvoice)
                        <a href="{{ url('/customer/invoice') }}/{{ $views->id }}" class="button btn btn-outline-success m-2">Invoice</a>
                        <a href="{{ route('customer.work.order.pdf', ['id' => $views->id]) }}" class="button btn btn-outline-success m-2">Download Pdf</a>
                        @else
                        @endif
                        @if($btnDispatch || $btnOpen)
                        <a href="{{ url('/customer/order/edit') }}/{{ $views->id }}" class="button btn btn-outline-danger m-2">Break Order</a>
                        @else
                        @endif
                        @if($btnClosed)
                        <a href="{{ route('customer.work.order.pdf', ['id' => $views->id]) }}" class="button btn btn-outline-success m-2">Download Pdf</a>
                        ...
                        @else
                        @endif
                        <a href="{{ url('/customer/customer/zone') }}/{{ @$views->customer->id }}" class="button btn btn-outline-primary m-2">Go Back</a>
                        @elseif(auth()->guard('admin')->user()->role_id == Status::DISPATCH_TEAM || auth()->guard('admin')->user()->role_id == Status::SALES_TEAM)
                        <div class="d-flex">
                            @if(auth()->guard('admin')->user()->role_id == Status::DISPATCH_TEAM)
                            <a href="{{ route('customer.work.order.pdf', ['id' => $views->id]) }}" class="button btn btn-outline-success m-2">Pdf</a>
                            @endif
                        </div>
                    </div>
                    @endif
            ...
            <div class="card-body">
                <section class="container">
                    ...
                    <nav class="navbar navbar-expand-lg" style="margin-top:-30px">
                        <div class="container-fluid">
                            <a class="navbar-brand" href="#"><img src="https://techyeahinc.com/assets/media/logo/tech_yeah_logo.png" alt="" style="width:170px;margin-left: -17px;"></a>
                            <div class="collapse navbar-collapse" id="navbarScroll">
                                ...
                                <form class="d-flex" role="search">
                                    <div>
                                        ...
                                        <h5 style="float: right" href="http://www.techyeahinc.com">www.techyeahinc.com</h5>
                                    </div>
                                </form>
                            </div>
                        </div>
                ...
                <section class="container">
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <img src="{{ asset('imgs/' . $imageName) }}" alt="Random Image" style="border: 2px solid #555; width: 350px; height: 250px; margin: 2px;">
                        @endforeach
                        @else
                ...
                <section class="container">
                    ...
                    @foreach ($wps as $wp)
                    <input type="text" class="form-input" value="({{$wp->date}})-{{$wp->work_perform}}">
                    @endforeach
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        ...
                        @foreach($wps as $wp)
                        <tr>
                            <td><input type="text" class="form-input" style="width: 100%;" value="({{$wp->date}})"></td>
                            <td><input type="text" class="form-input" style="width: 100%;" value="{{$wp->quantity}}"></td>
                            <td><input type="text" class="form-input" style="width: 100%;" value="{{$wp->description}}"></td>
                        </tr>
                        @endforeach
                    </table>
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="myForm">
                        <div class="row">
                            <input type="hidden" name="work_order_id" value="{{$views->id}}" id="work_order_id" readonly>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <textarea name="description" class="form-control" cols="30" rows="5" value="{{ old('description') }}" id="description"></textarea>
                                <span style="color:red; font-size:16px" id="description-error"></span>
                            </div>
                            <div class="form-group col-md-6">
                                <label for="work_request" class="form-label">Work request</label>
                                <textarea name="work_request" class="form-control" cols="30" rows="5" value="{{ old('work_request') }}" id="work_request"></textarea>
                                <span style="color:red; font-size:16px" id="work_request-error"></span>

                            </div>
                            <div class="form-group col-md-12">
                                <label for="work_perform" class="form-label">Work Perform</label>
                                <textarea name="work_perform" class="form-control" cols="30" rows="5" value="{{ old('work_perform') }}" id="work_perform"></textarea>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.message) {
                        ...
                        $("#quantity-error,#price-error,#description-error,#work_request-error,#work_perform-error").empty();
                        iziToast.success({
