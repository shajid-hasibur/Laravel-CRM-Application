...
class SupportTicketController extends Controller
{
    ...
    public function assignTicket()
    {
        ...
        if (request('search')) {
            $users = User::where('user_role', Status::FIELD_TECHNICIAN)->where('address->zip', 'like', '%' . request('search') . '%')->get();
        } else {
            $users = User::where('user_role', Status::FIELD_TECHNICIAN)->get();
        }
        return view('admin.support.assignticket', compact('pageTitle', 'users'));
    }
