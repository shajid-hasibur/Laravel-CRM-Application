                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{url('customer/order/view')}}/{{ $order->id}}">
                                        <span class="badge badge-light text-primary fs-6">{{$order->order_id}}</span>
                                    </a>
                                </td>
                                <td>{{$order->open_date}}</td>
                                <td><a href="{{url('customer/order/view')}}/{{ $order->id}}"><span class="badge badge-light text-primary fs-6">{{@$order->site->location}}</span></a></td>
                                <td>{{@$order->h_operation}}</td>
                                <td><span class="badge-success">{{@$order->site->address_1}}, {{@$order->site->address_2}}</span>
                                    <br><span class="badge-warning">{{@$order->site->city}}, {{@$order->site->state}},
                                        {{@$order->site->zipcode}}</span>
                                </td>
                                ...
                                <td>
                                    <div class=" dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <a class="dropdown-item" href="{{ url('/customer/order/edit') }}/{{ $order->id }}"><i class="fas fa-pencil-alt"></i> Edit</a>
                                            <a class="dropdown-item text-danger delete-button" href="{{ url('/customer/order/delete') }}/{{ $order->id }}" data-toggle="modal" data-target="#confirmationModal-{{ $order->id }}"><i class="fas fa-trash-alt"></i> Delete</a>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <a href="{{ url('/customer/order/delete') }}/{{ @$order->id }}" class="btn btn-danger">Delete</a>
                </div>
            </div>
        </div>
    </div>
